...
data |>
  ...
  cols_label(
    ...
    "av_cigprice" = "Price of Cigarettes"
  ) |>
  fmt_currency(
    columns = av_cigprice,
    currency = "USD",
...
```{r}
# Multiple Regression
multiple <- lm(bwght ~ cigs + faminc + fatheduc + motheduc + parity + male + white,
   data = data)

tidy(multiple) |>
  kable()
```
Suggested response to analysis questions:
The difference in the coefficient on cigs is 0.084. This is because the multiple regression controls for more factors that affect both `cigs` and `bwght`. The coefficient represents the average change in ounces of a baby's birth weight for the mother smoking one additional cigarette.
...
```{r}
lm(bwght ~ treat + faminc + fatheduc + motheduc + parity + male + white,
   data = data)
```
...
Suggested response to analysis questions:
The combination of smoker mother and male child seems to have the strongest effect on the strength of relationship between `faminc` and `bwght`. However, there is much to consider and keep in mind when considering this sort of data, and making this sort of inference. There is much we don't know about this dataset, such as the bias of its creators, the ethical nature of its creation, and much more that can seriously influence the reliability and interpretability of the dataset. So we should proceed with extreme care, and with knowledge of the limited nature of the inferences available to us, when examining data such as this. We would want to know a lot about the research and data collection methodology, the data ethics intentions of the dataset creators, and the overall sociological implications of the dataset in order to make more concrete inferences. And, importantly, we always want to keep respect and humanity at the top of mind when considering statistics that reflect real people.
...
Suggested response to analysis questions:
The q-q plot for smokers seems to contain less variation from the normal line, indicating that that subset may be more normally distributed. The q-q plot for non-smokers seems to indicate that there is a greater number of children of non-smokers being born at lower-than-expected weights. This could possibly be due to greater lifestyle similarities among smokers than among non-smokers, as smoking could be a unifying characteristic that leads to greater overall similarity in outcomes like child birth weight. However, this is just a possibility, and more sophisticated and ethical investigation would need to be done to make any firm conclusions here. 
